...
function estadoIndex(estado: TicketEstado) {
  // "listo_para_laminado" maps to same visual position as "en_impresion" (impresion done, laminado not started)
...
function estimatedRemainingMinutes(ticket: Ticket): number {
  ...
  switch (ticket.estado) {
    case "en_impresion": {
      ...
      const remainingImpresion = Math.max(0, ticket.tiempoImpresion - elapsedMin)
      return Math.ceil(remainingImpresion + ticket.tiempoLaminado)
    }
    case "listo_para_laminado":
      return Math.ceil(ticket.tiempoLaminado)
    case "en_laminado": {
      if (!ticket.inicioLaminado) return Math.ceil(ticket.tiempoLaminado)
      const startLam = new Date(ticket.inicioLaminado).getTime()
      const elapsedLam = (now - startLam) / 60000
      return Math.ceil(Math.max(0, ticket.tiempoLaminado - elapsedLam))
    }
    case "terminado":
      return 0
    default:
...
function ConsultaTicketCard({ ticket }: { ticket: Ticket }) {
  const currentIdx = estadoIndex(ticket.estado)
